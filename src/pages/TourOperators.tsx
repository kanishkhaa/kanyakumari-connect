...
import { ChevronDown, ChevronUp, Info, MessageSquare, Star, X } from "lucide-react";
...
const operatorDisplayOrder = [
  "fly-memories-travel-tours",
  "flybook-tours-travels",
  "kanyakumari-travels",
  "khalifa-travels",
  "kk-tours-travels",
  "subash-travels",
...
export default function TourOperators() {
  ...
  const operators = useMemo(() => {
    const uploadedOperators = [...getVendorOperators(), ...databaseOperators].filter((operator) => operator.verified);
    const byId = new Map(uploadedOperators.map((operator) => [operator.id, operator]));
    // The curated records contain the current, verified public contact links.
    fallbackOperators.forEach((operator) => byId.set(operator.id, operator));
    return Array.from(byId.values()).sort((a, b) => {
      const aOrder = operatorDisplayOrder.indexOf(a.id);
      const bOrder = operatorDisplayOrder.indexOf(b.id);
  ...
  return <div className="container mx-auto py-12">
    ...
    <div className="mt-10 space-y-6">{operators.map((operator) => <OperatorCard key={operator.id} operator={operator} reviewCount={getReviews(operator.id).length} onReview={() => setReviewFor(operator.id)} />)}</div>
...
function OperatorReviewModal({ operator, onClose, onSaved }: { operator: Operator; onClose: () => void; onSaved: () => void }) {
  ...
  const submitReview = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSaving(true);
    try { await saveReview({ listingId: operator.id, name: String(form.get("name")).trim(), rating, comment: String(form.get("comment")).trim(), createdAt: new Date().toISOString() }); onSaved(); } finally { setIsSaving(false); }
  };
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-foreground/50 p-4 sm:p-6" onClick={onClose}><form className="my-auto w-full max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-elevated" onClick={(event) => event.stopPropagation()} onSubmit={submitReview}>
    <div className="flex items-start justify-between border-b border-border px-5 py-4 sm:px-7"><div><p className="text-xs font-semibold uppercase tracking-widest text-primary">Traveller reviews</p><h2 className="mt-1 font-display text-2xl font-bold">{operator.name}</h2></div><Button type="button" variant="ghost" size="icon" aria-label="Close reviews" onClick={onClose}><X /></Button></div>
    <div className="max-h-[72vh] overflow-y-auto px-5 py-5 sm:px-7">
      <section className="rounded-xl border border-border bg-muted/30 p-4 sm:p-5"><div className="flex items-center gap-2 text-lg font-semibold"><span>Review summary</span><Info className="h-4 w-4 text-muted-foreground" /></div><div className="mt-4 grid gap-5 sm:grid-cols-[130px_1fr] sm:items-center"><div className="text-center sm:border-r sm:border-border sm:pr-5"><p className="font-display text-5xl font-bold leading-none">{reviewSummary.average.toFixed(1)}</p><StarRating rating={reviewSummary.average} size="sm" className="mt-2 justify-center" /><p className="mt-1 text-xs text-muted-foreground">{reviewSummary.total.toLocaleString()} reviews</p></div><div className="space-y-1.5">{[5, 4, 3, 2, 1].map((score) => <div key={score} className="grid grid-cols-[42px_1fr_28px] items-center gap-2 text-xs text-muted-foreground"><span>{score} star</span><div className="h-2.5 overflow-hidden rounded-full bg-border"><div className="h-full rounded-full bg-accent" style={{ width: `${reviewSummary.total ? (reviewSummary.distribution[score] / reviewSummary.total) * 100 : 0}%` }} /></div><span className="text-right">{reviewSummary.distribution[score]}</span></div>)}</div></div></section>
      <section className="mt-5 border-t border-border pt-5"><div className="flex items-center gap-2"><MessageSquare className="h-5 w-5 text-primary" /><h3 className="font-display text-xl font-semibold">Write a review</h3></div><p className="mt-1 text-sm text-muted-foreground">Share your tour experience to help future travellers.</p><div className="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><Field name="name" label="Your name" placeholder="Enter your name" required /><div><p className="text-sm font-semibold">Your rating</p><StarPicker value={rating} onChange={setRating} /></div></div><label className="mt-4 block text-sm font-semibold">Your review<textarea name="comment" rows={3} required minLength={5} placeholder="Tell travellers about your tour experience..." className="mt-1.5 w-full resize-y rounded-lg border border-input bg-background px-3 py-2.5 font-normal outline-none ring-offset-background focus:border-primary focus:ring-2 focus:ring-primary/20" /></label></section>
      <section className="mt-6 border-t border-border pt-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-display text-xl font-semibold">People's reviews</h3><p className="mt-0.5 text-sm text-muted-foreground">Reviews shared by travellers who used this operator.</p></div>{reviews.length > 4 && <Button type="button" variant="link" className="px-0" onClick={() => setShowAllReviews((value) => !value)}>{showAllReviews ? <>Show fewer <ChevronUp /></> : <>View all {reviews.length} reviews <ChevronDown /></>}</Button>}</div><div className="mt-4 grid gap-4 sm:grid-cols-2">{reviews.slice(0, showAllReviews ? undefined : 4).map((review, index) => <ReviewCard key={`${review.createdAt}-${index}`} review={review} />)}</div></section>
    </div>
